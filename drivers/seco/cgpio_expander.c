...
static unsigned int cgpio_exp_irq_startup (struct irq_data *data) {
	...
	if ( gpio_lock_as_irq (&cgpio_chip->gpio_chip, data->hwirq) ) {
...
static void cgpio_exp_irq_shutdown (struct irq_data *data) {
	...
	gpio_unlock_as_irq (&cgpio_chip->gpio_chip, data->hwirq);
}
...
static int cgpio_exp_irq_setup (struct cgpio_exp_chip *cgpio_chip) {
	...
	err = devm_request_threaded_irq (chip->dev, cgpio_chip->irq, NULL, cgpio_exp_hanlder_irq,
										IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
										dev_name(chip->dev), cgpio_chip);
	if (err != 0) {
...
static int cgpio_exp_remove (struct platform_device *pdev) {
	...
	status = gpiochip_remove (&cgpio_chip->gpio_chip);
	if ( status == 0 )
		kfree (cgpio_chip);
